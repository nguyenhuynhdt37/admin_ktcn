...
interface SeoAnalysisPanelProps {
  ...
  disabled?: boolean
  onAnalysisChange?: (analysis: SeoAnalysisResponse | null) => void
}
...
export function SeoAnalysisPanel({
  ...
  // Đồng bộ kết quả phân tích lên component cha để hiển thị gợi ý tại các input tương ứng
  useEffect(() => {
    onAnalysisChange?.(seo.analysis)
  }, [seo.analysis, onAnalysisChange])

  const statusConfig = seo.analysis ? seo.getStatusConfig(seo.analysis.status, seo.analysis.score) : null
  const strokeDashoffset = seo.analysis ? 251.2 - (251.2 * seo.analysis.score) / 100 : 251.2
  ...
  return (
    <div className="pt-4 pb-2 border-t border-border/40 space-y-4 text-left">
      ...
      <div className="space-y-1.5">
        <Label htmlFor="focusKeyword" className="text-xs font-semibold text-foreground/80">
        ...
        <div className="flex gap-2">
          <div className="relative flex-1">
            ...
            <Input
              id="focusKeyword"
              placeholder="Nhập từ khóa chính... (Ví dụ: tuyển sinh đại học)"
              value={focusKeyword}
      ...
      {/* Loading status */}
      {seo.isLoading && !seo.analysis && (
        <div className="flex items-center justify-center py-8 gap-2 border border-dashed rounded-lg bg-muted/5">
          <Loader2 className="h-4 w-4 animate-spin text-purple-600" />
          <span className="text-xs text-muted-foreground animate-pulse">AI đang phân tích bài viết của bạn...</span>
        </div>
      )}

      {/* Analysis Results */}
      {seo.analysis && !seo.isLoading && (
        <div className="space-y-3 animate-fade-in">
          <div className={cn('flex items-center gap-4 p-3.5 rounded-xl border', statusConfig?.bg, statusConfig?.border)}>
            ...
            <div className="space-y-0.5">
              ...
              <p className="text-[11px] text-muted-foreground leading-normal">
                Hệ thống Rule Check phát hiện {seo.analysis.issues.length} vấn đề cần lưu ý.
              </p>
            </div>
          </div>

          {/* Technical Issues Accordion */}
          {seo.analysis.issues.length > 0 && (
            <div className="border rounded-lg overflow-hidden bg-background/50">
              ...
              >
                <span>Lỗi kỹ thuật SEO cần sửa ({seo.analysis.issues.length})</span>
              ...
              {seo.isIssuesExpanded && (
                <div className="p-3 space-y-2 max-h-[180px] overflow-y-auto divide-y divide-border/40 text-[11px]">
                  {seo.analysis.issues.map((issue, idx) => (
          ...
          {/* Suggestions bullets */}
          {seo.analysis.suggestions.length > 0 && (
            <div className="space-y-1.5 p-3 rounded-lg border bg-muted/15 text-xs">
              ...
              <ul className="list-disc pl-4 space-y-1 text-muted-foreground leading-relaxed text-[11px]">
                {seo.analysis.suggestions.map((sug, idx) => (
                  <li key={idx}>{sug}</li>
                ))}
              </ul>
            </div>
          ...
          {/* AI Focus Keywords */}
          {seo.analysis.focus_keywords && seo.analysis.focus_keywords.length > 0 && (
            <div className="space-y-1.5 pt-3 border-t border-border/40">
              ...
              <div className="flex flex-wrap gap-1.5">
                {seo.analysis.focus_keywords.map((kw, idx) => (
                  <span
                    key={idx}
          ...
          {/* AI Internal Links */}
          {seo.analysis.internal_links && seo.analysis.internal_links.length > 0 && (
            <div className="space-y-2 pt-3 border-t border-border/40">
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-3 max-h-[220px] overflow-y-auto pr-1">
                {seo.analysis.internal_links.map((link, idx) => (
